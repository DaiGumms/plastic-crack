...
export const Warhammer40kIcon: React.FC<SvgIconProps> = (props) => (
  <SvgIcon {...props} viewBox="0 0 24 24">
    <path d="M12 2L3 7v10l9 5 9-5V7l-9-5zM6.5 8.5l5.5-3 5.5 3v7l-5.5 3-5.5-3v-7z" fill="currentColor" />
...
export const AgeOfSigmarIcon: React.FC<SvgIconProps> = (props) => (
  <SvgIcon {...props} viewBox="0 0 24 24">
    <path d="M12 2l2.5 7.5H22l-6 4.5 2.5 7.5L12 17l-6.5 4.5L8 14l-6-4.5h7.5L12 2z" fill="currentColor" />
  </SvgIcon>
);
...
export const getGameSystemIcon = (gameSystemId: string, props?: SvgIconProps) => {
  const gameSystem = getGameSystemById(gameSystemId);
  if (!gameSystem) return null;
  
  const IconComponent = gameSystem.icon;
  return <IconComponent {...props} sx={{ color: gameSystem.color, ...props?.sx }} />;
};

// Get game system from a collection object
export const getGameSystemFromCollection = (collection: { gameSystem?: { shortName: string } }): GameSystem | undefined => {
  if (!collection.gameSystem?.shortName) return undefined;
  const frontendId = DB_TO_FRONTEND_GAME_SYSTEM_MAP[collection.gameSystem.shortName];
  return getGameSystemById(frontendId);
};

// Get game system icon from a collection object
export const getGameSystemIconFromCollection = (collection: { gameSystem?: { shortName: string } }, props?: SvgIconProps) => {
...
export const getGameSystemDbId = async (frontendId: string): Promise<string> => {
