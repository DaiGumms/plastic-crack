...
vi.mock('../DragDropUpload', () => ({
  DragDropUpload: ({
    ...
    <div data-testid='drag-drop-upload'>
      <button
        onClick={() =>
          onFilesChange([{ id: '1', file: new File(['test'], 'test.jpg') }])
        }
      >
        Add File
      </button>
      <button
        onClick={() =>
          onUploadComplete([
            { id: '1', status: 'success', result: { url: 'test-url' } },
          ])
        }
      >
        Complete Upload
...
describe('UploadDialog', () => {
  ...
  it('renders when open', () => {
    ...
    expect(
      screen.getByText(/upload photos of your painted models/i)
    ).toBeInTheDocument();
  });
  ...
  it('shows correct title for avatar upload', () => {
    ...
    expect(
      screen.getByText(/only one image can be selected/i)
    ).toBeInTheDocument();
  });

  it('shows correct title for collection thumbnail', () => {
    render(
      <UploadDialog {...defaultProps} uploadType='collection-thumbnail' />
    );
  ...
  it('handles upload completion', async () => {
    ...
    render(
      <UploadDialog {...defaultProps} onUploadComplete={onUploadComplete} />
    );
