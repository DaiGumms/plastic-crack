...
The Plastic Crack application implements a comprehensive role-based access control (RBAC) system with JWT authentication and fine-grained permissions. This system was implemented as part of Issue #7.
...
router.delete('/users/:id', authenticateToken, requireRole(UserRole.ADMIN, UserRole.SUPER_ADMIN), handler);
```
...
router.get('/analytics', authenticateToken, requireRoleOrPermission([UserRole.ADMIN], ['analytics:view']), handler);
```
...
router.delete('/collections/:id', authenticateToken, requireResourcePermission('collection', 'delete'), handler);
```
